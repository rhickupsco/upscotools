...
Module Common
    Private Function sqlGeneratorForJumbo() As String
        Dim Sql As String

        Sql = "SELECT wom.QtyPlanned, wom.WODueDate, wood.StepNumber, wom.Status, " +
                ...
                " From  WO1_WorkOrderMaster wom, WO3_WorkOrderOperationDetail wood" +
                 " Where (wom.WorkOrder = wood.WorkOrder And wom.SubstepPrefix = wood.SubstepPrefix) " +
                   ...
                   "AND (wood.StepNumber In ('0010','0020','0023','0030','0040','0050')) ORDER BY wom.WODueDate ASC"
        Return Sql
    End Function
    ...
    Public Function GetTestData(ByVal stepNumber As String) As DataTable
        ...
        Dim sql As String
        sql = sqlGeneratorForJumbo(stepNumber)
    ...
    Public Function GetMonthsBack() As String
        Dim StartDate As New DateTime()
        StartDate = System.DateTime.Now()
        StartDate = DateAdd(DateInterval.Day, -360, StartDate)
